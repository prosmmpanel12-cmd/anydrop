 ...
 *
 * Deliberately still returns archived coupons (migration 27, doc 22
 * follow-up) rather than filtering them out server-side — the app's
 * CouponAdapter groups active vs. archived into two sections client-side
 * (so a restaurant can still see/unarchive an archived code from the
 * same screen) rather than needing a second endpoint or a query param.
 */
...
$stmt = $db->prepare(
    'SELECT c.id, c.code, c.discount_type, c.discount_value,
            ...
            c.is_active, c.is_public, c.is_archived,
