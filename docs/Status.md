...
## Customer App Restaurant & Menu UI refresh, continued same session (2026-09-01)

Continued the white+orange UI pass into the Restaurant & Menu flow,
same session as the Home pass above. Two files:
`activity_restaurant_detail.xml` and `item_menu_item.xml`.

- `btnBack` on the restaurant detail cover image: solid-orange square
  → white circle with dark icon, matching the bookmark/share buttons
  already next to it (sized/margined to match them exactly).
- `detailOfferBadge`: hardcoded black-translucent box → the same
  orange ribbon drawable (`bg_discount_badge_corner`) the restaurant
  list card badge already uses.
- `item_menu_item.xml`'s `itemDiscountBadge`: moved from bottom-end to
  top-start, matching the restaurant-card badge's corner from the
  earlier Home pass — all three discount-badge locations in the app
  (restaurant card, restaurant detail header, menu item card) now
  share one consistent look and position.

Deliberately left alone: the green rating pill (existing accent color,
not part of this white/orange pass, flagged rather than changed) and
the tab row / offers strip (already using the app's existing chip/card
primitives). No view IDs touched — see recall.md's matching entry for
full detail.

Not build/device-verified — same standing sandbox limitation. Cart &
Checkout is next if the owner wants to continue this UI pass.
